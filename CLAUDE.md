...
Any new or modified component must include `data-testid` attributes on interactive elements and assertion targets. Follow the convention in `.claude/skills/summit-testids/SKILL.md`. For lists and indexed items, use the `parent-{index}` pattern with 0-based indexing (e.g., `search-result-0`, `search-result-1`).
