...
export async function POST(request: NextRequest) {
  ...
  const previousWeekEntry = (() => {
    // Get the previous week's target to find its week_start
  ...
  // Check if any dimension has been behind for 2+ weeks (for "Consider Adjusting" section)
  const recentHistory = allEntries
    .filter((e: Record<string, unknown>) => e.change_reason === "weekly_rating")
    .slice(0, 3); // Last 3 weeks

  const consecutiveBehind: Record<string, number> = {};
  for (const dim of DIMENSIONS) {
    consecutiveBehind[dim] = 0;
  }
