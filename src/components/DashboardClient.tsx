...
interface DashboardData {
  ...
  validatedObjective: ValidatedObjective | null;
}
...
export default function DashboardClient() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      // If we have an active plan, fetch data based on it
      if (activePlanId) {
        ...
        const plan = planData as TrainingPlan | null;
        if (!plan) { setData({ objective: null, assessment: null, plan: null, currentWeekTarget: null, validatedObjective: null }); setLoading(false); return; }

        // Parallel fetch
        const assessmentQuery = plan.assessment_id
          ? supabase.from("assessments").select("*").eq("id", plan.assessment_id).single()
          : Promise.resolve({ data: null });

        const [objResult, weekResult, assessResult] = await Promise.all([
          supabase.from("objectives").select("*").eq("id", plan.objective_id).single(),
          supabase.from("weekly_targets").select("*").eq("plan_id", plan.id).order("week_number"),
          assessmentQuery,
        ]);

        const objective = objResult.data as Objective | null;
        const assessment = assessResult.data as Assessment | null;
        const weeks = (weekResult.data as WeeklyTarget[]) || [];
        const currentWeekTarget = weeks.find(w => w.week_number === plan.current_week_number) || weeks[0] || null;
        ...
        setData({ objective, assessment, plan, currentWeekTarget, validatedObjective });
      } else {
        // No active plan — check if user has any objectives
        ...
        const objective = (objectives as Objective[] | null)?.[0] || null;

        if (!objective) {
          setData({ objective: null, assessment: null, plan: null, currentWeekTarget: null, validatedObjective: null });
        } else {
          // Check for assessment on this objective
          ...
          const assessment = (assessments as Assessment[] | null)?.[0] || null;
          setData({ objective, assessment, plan: null, currentWeekTarget: null, validatedObjective: null });
        }
      }
      setLoading(false);
    }
  ...
  const { objective: activeObjective, assessment: objectiveAssessment, plan: activePlan, currentWeekTarget, validatedObjective } = data || {};
