...
- Climbing/Technical grade: First compute the overshoot grade: 1 sub-grade above the objective's outdoor lead grade (e.g., 5.10a route → 5.10b overshoot). Then convert to match the benchmark exercise type using these offsets (letter grades easier than outdoor lead): outdoor top-rope +3, indoor lead +4, indoor top-rope +6. Example: if the benchmark is "Top-Rope Climbing Assessment" and the route is 5.10a outdoor lead → overshoot to 5.10b → convert to outdoor top-rope (+3) = 5.11a top-rope graduation target.
...
TRAINING OVERSHOOT: The graduation benchmarks already include overshoot targets above the objective's actual requirements. Weekly sessions should progress toward these higher targets. By the final weeks, cardio sessions should reach ~130% of the objective's distance and elevation, and climbing sessions should be at the grade specified in the graduation benchmarks (which already accounts for overshoot and style conversion). Strength, flexibility, and pack weight stay at objective-level requirements.
...
GYM GRADE CONVERSION (applies to all gym climbing alternatives): Indoor gym grades are significantly HIGHER than outdoor grades. Convert from outdoor lead grade: indoor lead = outdoor lead + 4 letter grades, indoor top-rope = outdoor lead + 6 letter grades. Example: 5.9 outdoor lead = 5.10d indoor lead = 5.11b indoor top-rope. Do NOT use outdoor grades for gym sessions.
